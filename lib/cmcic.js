...
CMCIC.prototype.transaction.prototype.getDate = function() {
	...
	if (this.date === '') {
		this.date = day + "/" + month + "/" + year + ":" + hour + ":" + minute + ":" + second;
	}
};
...
CMCIC.prototype.transaction.prototype.stringifyTexteLibre = function() {
	...
	if (result != '' || (typeof(this.data.texteLibre) == 'object' && JSON.stringify(this.data.texteLibre).length == 2)) {
		this.data.texteLibre = result;
	}
};

CMCIC.prototype.transaction.prototype.form = function(id, autosubmit) {
	var texteLibre = this.getStringFromTexteLibre(),
	this.getDate(),
	mac = this.calculateMAC(this.dataToSend()),
