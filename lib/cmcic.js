...
CMCIC.prototype.tpe.prototype.setServer = function(bankName) {
	...
	this.postOptions.host = server;
	this.postOptions.host.replace('https://', '');
	if (this.tpe.test) {
		server += "/test";
		this.postOptions.path = '/test/paiement.cgi';
	}
	if (!this.tpe.test) {
		this.postOptions.path = '/paiement.cgi';
	}
	server += "/paiement.cgi";
...
CMCIC.prototype.tpe.prototype.checkTransactionReturn = function(transactionData) {
	...
	if (mac.toUpperCase() === transactionData.MAC.toUpperCase() && (transactionData['code-retour'] === 'paiement' ||  transactionData['code-retour'] === 'payetest')) {
		var request = http.request(this.postOptions);
		request.write('version=2\ncdr=0');
		request.end();
		...
		var request = http.request(this.postOptions, function (response) {
		  var str = '';
